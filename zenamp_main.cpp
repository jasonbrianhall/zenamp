...
void setup_windows_single_instance(AudioPlayer *player) {
    ...
    // There's no gdk_win32_surface_get_handle() function; the win32 backend
    // only exposes the HWND via the GDK_SURFACE_HWND() macro in gdkwin32.h.
    GdkSurface *gdk_surface = gtk_native_get_surface(GTK_NATIVE(player->window));
    if (gdk_surface) {
        HWND hwnd = GDK_SURFACE_HWND(gdk_surface);
