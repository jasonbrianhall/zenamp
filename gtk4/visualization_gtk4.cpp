...
Visualizer* visualizer_new(void) {
    ...
    // Mouse/scroll input in GTK4 comes from event controllers (GtkGestureClick,
    // GtkEventControllerMotion, GtkEventControllerScroll) attached to the
    // drawing area where it's built into the layout, not from an event mask here.
    ...
    // Connect signals. GTK4 drives cairo drawing through a draw-func on the
    // GtkDrawingArea itself (width/height come in as parameters on every call),
    // which also replaces the old "configure-event" handler that used to
    // stash vis->width/height and rebuild a backing surface.
    gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(vis->drawing_area),
                                    on_visualizer_draw, vis, NULL);
...
void visualizer_set_type(Visualizer *vis, VisualizationType type) {
    if (vis) {
        ...
        // GTK4 removed GtkContainer (and gtk_container_get_children with it) -
        // widgets are walked directly via first-child/next-sibling instead.
        if (player && player->vis_controls) {
            for (GtkWidget *widget = gtk_widget_get_first_child(player->vis_controls);
                 widget != NULL;
...
void on_visualizer_draw(GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer user_data) {
    Visualizer *vis = (Visualizer*)user_data;

    // GTK4 hands us the current size on every draw call instead of a separate
    // configure-event, so this is where vis->width/height now get updated.
    vis->width = width;
    vis->height = height;
...
gboolean visualizer_timer_callback(gpointer user_data) {
    ...
    if (vis->enabled) {
        ...
        // Check if window is visible on screen. GTK4 replaces per-widget
        // GdkWindow with GdkSurface (only top-levels have one, via GtkNative),
        // and GDK_WINDOW_STATE_ICONIFIED becomes GDK_TOPLEVEL_STATE_MINIMIZED
        // on a GdkToplevel.
        GdkSurface *gdk_surface = gtk_native_get_surface(GTK_NATIVE(player->window));
        bool is_minimized = gdk_surface &&
                           (gdk_toplevel_get_state(GDK_TOPLEVEL(gdk_surface)) & GDK_TOPLEVEL_STATE_MINIMIZED);
        bool is_visible = gtk_widget_get_visible(player->window) &&
                         gtk_widget_get_mapped(player->window) &&
                         !is_minimized;
...
// This grabs the first one as a stand-in "primary monitor". Caller must
// g_object_unref() a non-NULL result (unlike gdk_display_get_monitor_at_surface,
// which returns a borrowed reference).
...
GtkWidget* create_visualization_controls(Visualizer *vis) {
    // Get screen info to decide control layout. GTK4 removed GdkScreen -
    // monitor geometry now comes from GdkMonitor, and it's already reported
    // in logical (scale-adjusted) pixels, so no manual scale division needed.
    ...
    bool monitor_owned = !win_surface; // fallback path returns an owned ref; the surface path doesn't
    GdkMonitor *monitor = win_surface
        ? gdk_display_get_monitor_at_surface(display, win_surface)
        : gtk4_get_primary_monitor(display);
    GdkRectangle monitor_geometry = {0, 0, 1024, 768};
    if (monitor) {
    ...
    if (!use_compact_controls) {
        ...
        gtk_box_append(GTK_BOX(controls_box), type_label);
    }
    ...
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(type_combo), "Floppy Fish (i) (not music interactive)");
    ...
    gtk_box_append(GTK_BOX(controls_box), sens_scale);
