...
// on_visualizer_configure is gone in GTK4 - on_visualizer_draw now receives
// width/height directly on every call and updates vis->width/height itself.
...
// (on_queue_focus_in was declared here but never defined or called anywhere
// in the codebase - removed rather than inventing a GTK4 signature for it.
// If it's meant to exist, GTK4's equivalent is GtkEventControllerFocus's
// "enter" signal: void (*)(GtkEventControllerFocus*, gpointer).)
...
// NOTE(gtk4): these are defined in layout.cpp (not yet converted), wired to a
// GtkEventBox there today - GtkEventBox is gone in GTK4, so wherever they get
// connected needs to switch to controllers added directly to the drawing
// area via gtk_widget_add_controller(). Signal signatures below match what
// GTK4 actually calls back with:
//   "enter-notify-event"/"leave-notify-event"  -> GtkEventControllerMotion's "enter"/"leave" (void, no gboolean)
//   "button-release-event"                     -> GtkGestureClick's "released" (void, no gboolean)
//   "motion-notify-event"                      -> GtkEventControllerMotion's "motion" (void, no gboolean)
//   "scroll-event"                              -> GtkEventControllerScroll's "scroll" (gboolean - genuinely returns one)
