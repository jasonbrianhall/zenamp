...
// instead of blocking the caller. (Same helper as zenamp_main.cpp's, which
// is static/internal-linkage there and not visible from this translation unit.)
...
// "key-press-event" -> GtkEventControllerKey's "key-pressed" signal in GTK4;
// keyval/keycode/state arrive as direct parameters instead of a GdkEventKey.
gboolean on_key_press_event(GtkEventControllerKey *controller, guint keyval, guint keycode, GdkModifierType state, gpointer user_data) {
    ...
    switch (keyval) {
        ...
        case GDK_KEY_F10:
            {
                ...
                if (is_visible) {
                    ...
                    if (player->layout.toggle_queue_menu_item) {
                        ...
                        // NOTE(gtk4): GtkCheckMenuItem is gone entirely. This
                        // assumes toggle_queue_menu_item becomes a GtkCheckButton
                        // once layout.cpp's menu gets rebuilt (GTK4 menu bars are
                        // GMenu/GAction-backed and don't expose settable widget
                        // state directly) - update this if it ends up being a
                        // GSimpleAction instead (g_simple_action_set_state()).
                        gtk_check_button_set_active(
                            GTK_CHECK_BUTTON(player->layout.toggle_queue_menu_item), 
                            FALSE);
        ...
        case GDK_KEY_f:
        {
            gboolean alt_pressed = (state & GDK_ALT_MASK) != 0;
            if (!alt_pressed) {
                toggle_vis_fullscreen(player);
                return TRUE;
            }
        ...
        case GDK_KEY_Q:
            if (ctrl_pressed) {
                // gtk_main_quit() is gone in GTK4. Closing the window triggers
                // the same "close-request" cleanup path as clicking the X
                // button (which stops the GMainLoop in zenamp_main.cpp), so
                // route through that instead of duplicating quit logic here.
        ...
        case GDK_KEY_9:
            {
                int queue_pos = keyval - GDK_KEY_1;
...
void toggle_fullscreen(AudioPlayer *player) {
    ...
    // window state ("iconified", "fullscreen", etc.) moves from GdkWindowState
    // to GdkToplevelState via gdk_toplevel_get_state() - same pattern already
    ...
    GdkToplevelState state = gdk_toplevel_get_state(GDK_TOPLEVEL(gdk_surface));
...
// "key-press-event" -> GtkEventControllerKey's "key-pressed" signal, matching
// the declaration in audio_player.h and the call site in
...
void show_keyboard_help(AudioPlayer *player) {
    ...
    int border = use_compact_dialog ? 10 : 15;
    gtk_widget_set_margin_start(content_area, border);
    gtk_widget_set_margin_end(content_area, border);
    gtk_widget_set_margin_top(content_area, border);
    gtk_widget_set_margin_bottom(content_area, border);
    ...
    if (use_compact_dialog) {
        ...
        // gtk_label_set_line_wrap() is renamed to gtk_label_set_wrap() in GTK4.
        gtk_label_set_wrap(GTK_LABEL(label), TRUE);
        // gtk_scrolled_window_add_with_viewport() is gone - GTK4 widgets handle
        // scrolling adjustments natively, so the scrolled window takes its
        // child directly.
    ...
    // instead of blocking (same pattern used throughout zenamp_main.cpp).
...
void setup_keyboard_shortcuts(AudioPlayer *player) {
    ...
    // "key-press-event" -> GtkEventControllerKey added via gtk_widget_add_controller().
    GtkEventController *key_controller = gtk_event_controller_key_new();
    g_signal_connect(key_controller, "key-pressed",
                     G_CALLBACK(on_key_press_event), player);
    gtk_widget_add_controller(player->window, key_controller);
}
