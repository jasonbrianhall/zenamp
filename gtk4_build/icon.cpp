...
// Scales a single animation frame to the widget's configured display size.
// The embedded animation frames are natively only 64x64 (see icon.h's
// header comment) - that's a genuinely tiny icon on modern screens, and
// nothing was ever scaling frames up to match layout.cpp's icon_size
// before this. gdk_pixbuf_animation_iter_get_pixbuf() returns a borrowed
// reference (must not be unref'd); this always returns a new, caller-owned
// pixbuf so the calling code has one consistent ownership rule regardless
// of whether scaling actually happened.
static GdkPixbuf* scale_frame_to_display_size(GdkPixbuf *frame, gint display_size) {
    if (!frame) return NULL;
    if (display_size <= 0) {
        return (GdkPixbuf*)g_object_ref(frame);
    }
    if (gdk_pixbuf_get_width(frame) == display_size && gdk_pixbuf_get_height(frame) == display_size) {
        return (GdkPixbuf*)g_object_ref(frame);
    }
    return gdk_pixbuf_scale_simple(frame, display_size, display_size, GDK_INTERP_BILINEAR);
}
...
gboolean on_icon_animation_frame(gpointer user_data) {
    ...
    if (frame) {
        GdkPixbuf *scaled = scale_frame_to_display_size(frame, state->display_size);
        if (scaled) {
            gtk_image_set_from_pixbuf(state->image_widget, scaled);
...
IconAnimationState* init_icon_animation(GtkImage *image_widget, gint display_size) {
    ...
    // Get first frame and store a scaled, owned copy of it
    GdkPixbuf *raw_first_frame = gdk_pixbuf_animation_iter_get_pixbuf(state->iter);
    state->first_frame = scale_frame_to_display_size(raw_first_frame, display_size);
